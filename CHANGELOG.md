...
- **Maintainability (Matt-Pocock review leftovers)**: the collector now reads the version from the `VERSION` file at startup (single source of truth; a Linux test keeps the standalone-copy fallback constant in sync — no more Shotgun Surgery across `VERSION`/`$ScriptVersion`/CI pin); JSON manifests are written UTF-8 **without BOM** via `WriteAllText` so the report contract is byte-identical on PS 5.1 and pwsh 7 (the CSV export keeps its BOM — Excel needs it).
